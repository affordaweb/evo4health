      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            {items.map((item) => (
              <div key={item.id} className="bg-white rounded-2xl p-6 shadow-sm flex items-center space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-primary-700 font-bold">{formatPrice(item.price)}</p>
                </div>
                <div className="flex items-center space-x-2">
                  <button onClick={() => updateQty(item.id, item.quantity - 1)} className="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-colors">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm h-fit sticky top-24">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between text-slate-600">
                <span>Shipping</span>
                <span>{total >= 75 ? <span className="text-green-600 font-medium">FREE</span> : formatPrice(9.99)}</span>
              </div>
              {total < 75 && (
                <p className="text-sm text-slate-500">Add {formatPrice(75 - total)} more for free shipping!</p>
              )}
              <div className="border-t pt-3 flex justify-between font-bold text-lg text-slate-900">
                <span>Total</span>
                <span className="text-primary-700">{formatPrice(total >= 75 ? total : total + 9.99)}</span>
              </div>
            </div>
